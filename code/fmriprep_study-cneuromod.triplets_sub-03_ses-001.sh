...
#SBATCH --output=/scratch/bpinsard/triplets.fmriprep/code/fmriprep_study-cneuromod.triplets_sub-03_ses-001.out
#SBATCH --error=/scratch/bpinsard/triplets.fmriprep/code/fmriprep_study-cneuromod.triplets_sub-03_ses-001.err
#SBATCH --time=8:00:00
...
if [ -d sourcedata/freesurfer ] ; then
    flock --verbose /lustre03/project/rrg-pbellec/ria-beluga/alias/cneuromod.triplets.fmriprep/.datalad_lock datalad push -J 4 -d sourcedata/freesurfer $LOCAL_DATASET --to origin
fi 
if [ -e $LOCAL_DATASET/workdir/fmriprep_wf/resource_monitor.json ] ; then cp $LOCAL_DATASET/workdir/fmriprep_wf/resource_monitor.json /scratch/bpinsard/fmriprep_study-cneuromod.triplets_sub-03_ses-001_resource_monitor.json ; fi 
if [ $fmriprep_exitcode -ne 0 ] ; then cp -R $LOCAL_DATASET /scratch/bpinsard/fmriprep_study-cneuromod.triplets_sub-03_ses-001 ; fi 
